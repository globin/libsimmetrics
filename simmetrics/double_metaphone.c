...
metastring *new_meta_string(const char *init_str) {
    metastring *s;
    char empty_string[] = "";
...
int is_vowel(metastring *s, size_t pos) {
    char c;
...
char get_at(metastring *s, size_t pos) {
    if ((pos < 0) || (pos >= s->length))
        return ('\0');
 ...
 */
int str_at(metastring *s, size_t start, size_t length, ...) {
    char *test;
    char *pos;
    va_list ap;
